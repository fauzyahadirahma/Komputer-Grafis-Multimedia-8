...
public class Cube3DTest extends JFrame
 ...
 MouseWheelListener {
    ...
    public Cube3DTest() {
        super("3D Animasi Cube Mouse dan Rotasi");
        
        /* view setup*/
        angel = (float) Math.toRadians(40);
        distance = (width / 2) / (float) (Math.tan(angel / 2));
    ...
    public void rotateVector(Vector3D p, float thetaX,
            float thetaY, float thetaZ) {
                ...
                p.y = (float) ((aY-camY)*Math.cos(thetaX)-(aZ-camZ)*
                        Math.sin(thetaX));
                p.z = (float) ((aY-camY)*Math.sin(thetaX)+(aZ-camZ)*
                        Math.cos(thetaX));
                
                aX = p.x;
                aZ = p.z;
                
                p.x = (float) ((aX-camX)*Math.cos(thetaY)+(aZ-camZ)*
                        Math.sin(thetaY));
                p.z = (float) (-(aX-camX)*Math.sin(thetaY)+(aZ-camZ)*
                        Math.cos(thetaY));
                
                aY = p.y;
                aX = p.x;
                
                p.x = (float) ((aX-camX)*Math.cos(thetaZ)-(aY-camY)*
                        Math.sin(thetaZ));
                p.y = (float) ((aY-camY)*Math.cos(thetaZ)+(aX-camX)*
                        Math.sin(thetaZ));
    }
}
